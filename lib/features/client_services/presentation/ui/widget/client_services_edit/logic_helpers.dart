...
extension _ClientServicesEditLogic on _ClientServicesEditPageState {
  ...
  Future<_PickedImage?> _pickImage() async {
    ...
    input.onChange.listen((event) {
      ...
      print(
          '[_pickImage] file selected: name=${file.name} type=${file.type} size=${file.size}');

      if (!file.name.toLowerCase().endsWith('.svg') &&
          file.type != 'image/svg+xml') {
        print(
            '[_pickImage] ❌ rejected: not an SVG (name=${file.name}, type=${file.type})');
        if (!completed) {
          completed = true;
          completer.complete(null);
        }
      ...
      reader.onLoadEnd.listen((_) {
        ...
        print(
            '[_pickImage] onLoadEnd: result runtimeType = ${result.runtimeType}');
        if (!completed) {
          completed = true;
          if (result is ByteBuffer) {
            ...
            print(
                '[_pickImage] ❌ unknown result type: ${result.runtimeType}');
            completer.complete(null);
          }
        }
      });
    ...
    print(
        '[_pickImage] ◀ DONE — result: ${result == null ? 'null' : 'bytes=${result.bytes?.length} url=${result.url}'}');
    return result;
  }
