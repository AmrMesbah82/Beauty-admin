...
class _TermsEditPageState extends State<TermsEditPage> {
  ...
  bool get _isDirty {
    ...
    if (_privacyDescEnCtrl.text.trim() != _origPrivacyDescEn.trim()) return true;
    if (_privacyDescArCtrl.text.trim() != _origPrivacyDescAr.trim()) return true;
  ...
  bool _hasFieldError(TextEditingController ctrl, TextDirection dir) {
    ...
    if (text.trim().isEmpty) return false; // empty is handled by _allFieldsFilled
  ...
  Widget _buildForm() {
    ...
    return Column(
      ...
      children: [
        ...
        Row(children: [
          Expanded(child: _btn(
              label: 'Preview', color: const Color(0xFFD16F9A),
              onTap: _onPreview)),
          SizedBox(width: 16.w),
          Expanded(child: _btn(
              label: 'Publish',
        ...
        Row(
          children: [
            Expanded(
              child: _btn(label: 'Discard', color: const Color(0xFF9E9E9E),
